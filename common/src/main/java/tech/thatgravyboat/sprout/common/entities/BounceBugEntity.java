...
public class BounceBugEntity extends TamableAnimal implements IAnimatable, Herbivore {
    ...
    private static final EntityDataAccessor<Boolean> BOUNCE_BUG_VARIANT = SynchedEntityData.defineId(BounceBugEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    protected void defineSynchedData() {
        ...
        this.getEntityData().define(BOUNCE_BUG_VARIANT, true);
    ...
    public boolean isNearShroom() {
        return shroomPos != null && shroomPos.distSqr(this.blockPosition()) < 9;
    }
    ...
    private void setBounceBugVariant(BounceBugVariant variant) {
        getEntityData().set(BOUNCE_BUG_VARIANT, variant == BounceBugVariant.WARPED);
    }

    public BounceBugVariant getBounceBugVariant() {
        return getEntityData().get(BOUNCE_BUG_VARIANT) ? BounceBugVariant.WARPED : BounceBugVariant.CRIMSON;
    }
